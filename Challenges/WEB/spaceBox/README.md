...
- If you login with user hamza it's better to change text at index.php to show login as hamza, hide the login, add logout and edit the welcome message
- Posts upload need a fix : file_put_contents("posts/".$filename."php", $content); --> file_put_contents("posts/".$filename.".php", $content);
- some typo fixes in differents files ex: dashboaed, welcom, forbiden..
